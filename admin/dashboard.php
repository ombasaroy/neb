...
// }
    ...
    $counter_sql_president = "SELECT COUNT(*) AS counter_president  FROM aspirants WHERE position='president'"; 
    $counter_sql_governor = "SELECT COUNT(*) AS counter_governor  FROM aspirants WHERE position='governor'";
    $counter_sql_senate = "SELECT COUNT(*) AS counter_senate  FROM aspirants WHERE position='senate'"; 
    $counter_sql_wrep = "SELECT COUNT(*) AS counter_wrep  FROM aspirants WHERE position='WREP'";
    $counter_sql_mna = "SELECT COUNT(*) AS counter_mna  FROM aspirants WHERE position='MNA'"; 
    $counter_sql_mca = "SELECT COUNT(*) AS counter_mca  FROM aspirants WHERE position='MCA'";
    $counter_sql_party_list = "SELECT COUNT(*) AS counter_party_list  FROM aspirants WHERE position='party list'";

    $result_president = $conn->query($counter_sql_president) or die($mysqli->error);
    $result_governor = $conn->query($counter_sql_governor) or die($mysqli->error);
    $result_senate = $conn->query($counter_sql_senate) or die($mysqli->error);
    $result_wrep = $conn->query($counter_sql_wrep) or die($mysqli->error);
    $result_mna = $conn->query($counter_sql_mna) or die($mysqli->error);
    $result_mca = $conn->query($counter_sql_mca) or die($mysqli->error);
    $result_party_list = $conn->query($counter_sql_party_list) or die($mysqli->error);

    $row_president = $result_president->fetch_assoc();
    $row_governor = $result_governor->fetch_assoc();
    $row_senate = $result_senate->fetch_assoc();
    $row_wrep = $result_wrep->fetch_assoc();
    $row_mna = $result_mna->fetch_assoc();
    $row_mca = $result_mca->fetch_assoc();
    $row_party_list = $result_party_list->fetch_assoc();

    $president = $row_president['counter_president'];
    $governor = $row_governor['counter_governor']; 
    $senate = $row_senate['counter_senate'];
    $wrep = $row_wrep['counter_wrep']; 
    $mna = $row_mna['counter_mna'];
    $mca = $row_mca['counter_mca']; 
    $party_list = $row_party_list['counter_party_list']; 
